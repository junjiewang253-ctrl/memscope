...
def megatron_args_to_full_config(args) -> FullConfig:
    model = ModelConfig(
        ...
        num_kv_heads=getattr(args, "num_query_groups", getattr(args, "num_attention_heads", 1)), 
        ffn_hidden_size=getattr(args, "ffn_hidden_size", 4 * getattr(args, "hiddens_size", 1)), 
        vocab_size=getattr(args, "vocab_size", None) or getattr(args, "padded_vocab_size", 0), 
    ...
    zero_stage = 0
    if getattr(args, "used_distributed_optimizer", False):
        zero_stage = 1

    train = TrainConfig(
        micro_batch_size=getattr(args, "micro_batch_size", 1), 
        grad_accum_steps=1, # Megatron内部有microbatch体系，这里先近似
        ...
        activation_checkpointing=(getattr(args, "recompute_activations", False) 
